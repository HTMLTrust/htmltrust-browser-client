...
export interface IdentifierBindingOptions extends VerificationFetchOptions {
  ...
  now?: () => number;
  /** Cache lifetime in ms. Default 1h, matching this package's existing key-document caching recommendation. */
  maxAgeMs?: number;
...
function didWebDocumentUrl(keyid: string): { url: string; didPortion: string } | null {
  ...
  const url =
    pathParts.length === 0
      ...
      : `https://${authority.host}/${pathParts.map(encodeURIComponent).join("/")}/did.json`;
...
export async function checkKeyIdentifierBinding(
  ...
  cache.set(keyid, { result, expiresAt: now + maxAge });
  return result;
}
